...
executable_name = "FF_Gui_Windows"

# Définir le nom du dossier de destination
destination_folder_name = "FF_Gui_Windows_Folder"
...
if os.getenv('GITHUB_ACTIONS') == 'true':
    ...
    executable_path = os.path.join(dist_path, executable_name, executable_name + ".exe")
    # Chemin absolu du dossier _internal
    internal_folder_path = os.path.join(dist_path, executable_name, "_internal")
else:
    # Chemin du répertoire de distribution
...
# Chemin absolu du dossier final
final_folder_path = os.path.join(destination_folder_path, executable_name)

# Vérifier si le répertoire de l'exécutable existe, le créer s'il n'existe pas
...
if os.path.exists(executable_path):
    # Déplacer le fichier exécutable dans le dossier de l'exécutable
    shutil.move(executable_path, os.path.join(final_folder_path, executable_name + ".exe"))

    # Déplacer le dossier _internal dans le dossier de l'exécutable
    shutil.move(internal_folder_path, final_folder_path)

    # Compresser le dossier de destination
    shutil.make_archive(os.path.join(dist_path, destination_folder_name), "zip", destination_folder_path)
